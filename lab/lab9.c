...
TCP because it calls connect which means it is establishing a connection, the socket is also created
with SOCK_STREAM.

3. The client is going to send some data to the server. Where does it get this data from? How can
you tell in the code?
The data is from stdin, because the read line takes STDIN_FILENO.

4. How does the client program end? How can you tell that in the code?
The client program ends when the user sends EOF, you can tell through the while loop wich exists
