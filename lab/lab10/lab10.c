...
TCP Protocol, a socket is created then connect() establishs a client-server connection, messages are
sent with write.

2. What data is the client sending to the server?
5 strings are sent from client to server, padded with BUF_SIZE buffer.

Understanding the Server:
1. Explain the argument that the `run_acceptor` thread is passed as an argument.
The run_acceptor argument passes a pointer to the struct acceptor args.with a flag to stop the
acceptor, a pointer to the shared message list and a pointer to the lock mutex.

2. How are received messages stored?
Received messages are stored in a dynamically allocated shared linked list.

3. What does `main()` do with the received messages?
main() checks the list_handle_count until max clients are collected then it calls collect all to
print and free the messages..

4. How are threads used in this sample code?
main() uses one acceptor thread to accept client conections. The acceptor create a client thread
that reads the messages and adds it to the linked list.
